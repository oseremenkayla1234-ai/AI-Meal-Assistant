...
import { db, categorizeIngredient, DEFAULT_USER_ID, ADMIN_USER_ID } from './server/db.js';
import { generateSingleMeal, generateCookWithIngredients } from './server/gemini.js';
...
async function startServer() {
  ...
  app.get('/api/admin/stats', (req, res) => {
    try {
      ...
      const user = db.findUserById(userId);
      if (user?.role !== 'admin') {
        // Return stats anyway for demo mode or warning
      }
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`AI Meal Assistant server running on http://0.0.0.0:${PORT}`);
  });
}
